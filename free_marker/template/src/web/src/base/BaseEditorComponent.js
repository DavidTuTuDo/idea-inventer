...
class BaseEditorComponent extends BaseComponent {
    ...
    static getItemJobs(name) {
        return [{ name: "delete", loadingText: "正在刪除中", buttonText: `刪除${name}` }];
    }

    static getCollectionJobs(name = "") {
        return [
            { name: "create", loadingText: "正在新增中", buttonText: `新增${name}` },
            { name: "createA-E", loadingText: "正在新增中", buttonText: `新增${name} A-E` },
            { name: "create1-5", loadingText: "正在新增中", buttonText: `新增${name} 1-5` },
    ...
    renderEditorFunctionView(types, onClickedAsyncTask) {
        const self = this;
        return (
            <div className={"BaseEditorFunctionDiv"}>
                {types.map((type) => {
                    return (
                        <Button
                            key={type.name}
    ...
    async buildWatermarkBlob(origin) {
        ...
        switch (configOfWatermark.type) {
            case "text":
                ...
                const blobOfTextWatermark = await watermark([origin.blob]).blob(
                    functionNameOfText(configOfWatermark.src, configOfWatermark.textStyle, configOfWatermark.color, configOfWatermark.alpha)
                );
            ...
            case "image":
                ...
                const blobOfImageWatermark = await watermark([origin.url, configOfWatermark.src]).blob(functionNameOfImage(configOfWatermark.alpha));
        ...
        if (files.length === 1) {
            ...
            const task = async () => {
                if (self.currentUploadImagesTaskInfo.needWatermark) {
                    ...
                    const urlOfOrigin = await this.getStore().uploadStorageFile(file, self.currentUploadImagesTaskInfo.folderName);
                    ...
                    const url = await this.getStore().uploadStorageFile(file, self.currentUploadImagesTaskInfo.folderName);
            ...
            const task = async () => {
                ...
                for (const file of files) {
                    const remoteUrl = await this.getStore().uploadStorageFile(file, self.currentUploadImagesTaskInfo.folderName);
    ...
    renderItemEditorView(onEditClickedAsyncTask, hasPath, sign) {
        return this.renderEditorFunctionView(hasPath ? BaseEditorComponent.getItemRemoteJobs(sign) : BaseEditorComponent.getItemJobs(sign), onEditClickedAsyncTask);
    }

    renderCollectionEditorView(onCreateClickedAsyncTask, hasPath, sign) {
        return this.renderEditorFunctionView(hasPath ? BaseEditorComponent.getCollectionRemoteJobs(sign) : BaseEditorComponent.getCollectionJobs(sign), onCreateClickedAsyncTask);
    }
