        ...
        Util.syncDelay(1)
            ...
            .then((clazzItem) => {
                if (!Util.isUndefinedNullEmpty(clazzItem)) {
                    ...
                    self.setDatOfPeriodWithHours(
                        `${Util.getStringOfFormatTimestampRange(
                            this.normalizeTimestamp(clazzItem.startOfSpecificClass),
                            this.normalizeTimestamp(clazzItem.endOfSpecificClass)
                        )} (合計：${self.getStringOfHours(clazzItem)})`
                    );
