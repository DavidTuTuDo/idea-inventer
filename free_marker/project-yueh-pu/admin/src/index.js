...
(async () => {
    ...
    async function fetchTopSongsOfRank(n) {
        return Util.getArrayOfSize(_.orderBy(await database.fetchRecords('RANK', new Builder().gt('WEEK', 0).orderBy({ WEEK: 'ASC' }).stmt()), (each) => each.WEEK, 'ASC'), n);
    }
    ...
    async function deployMainPageHotRhythm(n) {
        ...
        if (_.size(ranks) > 5) {
            await api.submitHotRhythms(ranks.map((each) => {
                return {
                    name: each.name, singer: each.singer, indexOfSequence: each.WEEK, idOfGuitarPu: getDocumentId(each, guitars), uuidOfSong: each.url
                };
            }));
        }
    ...
    async function deployMainPageHotSingers(n) {
        ...
        if (_.size(singers) > 0) {
            await api.submitHotSingers(singers.map((each, index) => {
                return {
                    name: each.name, singer: each.singer, indexOfSequence: index, idOfSinger: each.id, statement: `作品 ${each.countsOfRhythm} 件`
                };
            }));
        }
    }
    ...
    async function accumulatePopularLevelOfSinger() {
        ...
        for (const singer of singers) {
            ...
            await database.updateRecords('SINGER', {
                popularLevel: popularLevel, songCounts: countsOfRhythm
        ...
        if (_.size(singers) > THRESHOLD_OF_BATCH_MODE) {
            ...
            for (const singer of singers) {
                ...
                if (resultOfSinger.succeed) {
                    await database.updateRecords('SINGER', { idOfRemote: resultOfSinger.value.id }, new Builder().equal('url', resultOfSinger.value.uuidOfSinger).stmt());
                }
    ...
    function getNormalizedSingerItem(singer) {
        return {
            name: singer.name, nicknames: singer.names.split(`#&#@#`).map(each => {
                return { statement: each };
            }), type: singer.type, uuidOfSinger: singer.url, popularLevel: singer.popularLevel, countsOfRhythm: singer.songCounts
        };
    }
    ...
    async function deployKeywords() {
        ...
        keywords.push(...singers.map((singer) => {
            return {
                // value: singer.name,
                label: singer.name, popularLevel: singer.popularLevel, type: 12, uid: singer.idOfRemote
        ...
        keywords.push(...rhythms.map((rhythm) => {
            return {
                // value: rhythm.singer,
                label: `${rhythm.singer}-${rhythm.name}`, popularLevel: rhythm.popularLevel, type: 11, uid: rhythm.idOfRhythm
                // extra: `11是代表rhythm,12代表singer`,
            };
        }));
            ...
            for (const tone of tones) {
                ...
                if (resultOfGuitar.succeed) {
                    const resultOfRhythm = await api.submitRhythmItem({
                        ...resultOfGuitar.value, idOfGuitarPu: resultOfGuitar.value.id
                    }, resultOfGuitar.value.id);
                    /** 更新idOfRemote到本端db */

                    await database.updateRecords('TONE', {
                        idOfRemote: resultOfGuitar.value.id, idOfRhythm: resultOfRhythm.value.id
    ...
    function refactorTone(string) {
        ...
        let latest = Util.replaceAllWithSets(origin, { from: '▲', to: '🌕' }, { from: '★', to: '🌓' }, { from: '△', to: '🌑' }, { from: '✩', to: '🌙' }, { from: '☆', to: '🌙' }, {
            from: '\\|',
            to: '།'
        }, { from: '\\[前奏\\]', to: '(開始)' }, { from: '\\[尾奏\\]', to: '(結束)' }, { from: '\\[間奏\\]', to: '(橋段)' }, { from: '\\[間奏1\\]', to: '(橋段I)' }, {
            from: '\\[間奏2\\]',
            to: '(橋段II)'
        }, { from: '\\[間奏3\\]', to: '(橋段III)' }, { from: '\\[間奏4\\]', to: '(橋段IV)' }, { from: '\\(End\\)', to: '(終止)' });
    ...
    function getSubmitGuitarPuItemWithNormalized(tone, singers) {
        ...
        if (Util.isUndefinedNullEmpty(objOfTones['原調']) || Util.isUndefinedNullEmpty(objOfTones['男調']) || Util.isUndefinedNullEmpty(objOfTones['女調'])) {
    ...
    async function submitShortcut() {
        ...
        await api.submitShortcuts([{
            title: '回到首頁', icon: 'muIcon:Bedtime', route: `route:main`, indexOfSequence: 0
        }, {
            title: '我的最愛', icon: 'muIcon:FavoriteBorder', route: `route:personalRhythm`, indexOfSequence: 1
        }, {
            title: '歷史搜尋', icon: 'muIcon:History', route: `route:historyRhythm`, indexOfSequence: 2
        }, {
            title: '編輯功能', icon: 'muIcon:EditRounded', indexOfSequence: 3, subs: [{
                title: '新增悅譜', icon: 'muIcon:LibraryMusic', route: `route:chordiventor`, indexOfSequence: 1
            }, {
                title: '我譜的譜', icon: 'muIcon:AddReactionRounded', route: `route:inventedOfPu`, indexOfSequence: 2
            }]
        }, {
            title: '相關網站', icon: 'muIcon:Whatshot', indexOfSequence: 4, subs: [{
                title: '91譜', icon: 'muIcon:School', route: 'path:https://www.91pu.com.tw/', indexOfSequence: 1
            }]
        }]);
    }
    ...
    function getCapoAndContextTonality(string) {
        ...
        const obj = {
            capo: getFirstNumber(string), tonalityOfContext: getStringOfBrace(string)
        };
        return obj;
    }
    ...
    async function syncSingerRemoteIdIntoLocalStorage() {
        ...
        for (const singer of singers) {
            await database.updateRecords('SINGER', { idOfRemote: singer.id }, new Builder().equal('url', singer.uuidOfSinger).stmt());
        }
    }
    ...
    async function syncRemoteIdWithToneAndRhythmIntoLocalStorage() {
        ...
        for (const guitar of guitars) {
            await database.updateRecords('TONE', { idOfRemote: guitar.id }, new Builder().equal('url', guitar.uuidOfSong).stmt());
        }

        const rhythms = await api.fetchRhythms();
        for (const rhythm of rhythms) {
            await database.updateRecords('TONE', { idOfRhythm: rhythm.id }, new Builder().equal('url', rhythm.uuidOfSong).stmt());
        }
    }
        ...
        for (const folder of Util.getChildPathByPath(pathOfPreludes)) {
            if (Util.isDirectory(folder.absolute) && _.size(Util.getChildPathByPath(folder.absolute)) > 1) {
                ...
                await database.lazyInsertRecord('TONE', {
                    ...record, pathOfPreludeC: urlOfC, pathOfPreludeG: urlOfG, hasPrelude: true
                });
    ...
    async function uploadFileToPublicStorage(pathOfLocalFile, destinationOfRemote) {
        ...
        const config = {
            action: 'read', expires: '02-14-2100'
        };

        const options = {
            destination: destinationOfRemote, preconditionOpts: {
    ...
    async function syncPreludeInfoToRemoteFirestore() {
        ...
        const itemsOfTone = pus.map(pu => {
            return {
                id: pu.idOfRemote, uid: pu.uid, hasPrelude: true, pathOfPreludeC: pu.pathOfPreludeC, pathOfPreludeG: pu.pathOfPreludeG
            };
        });

        const itemsOfRhythm = pus.map(pu => {
            return {
                id: pu.idOfRhythm, hasPrelude: true
            };
        });
    ...
    async function updateCPRT() {
        await api.submitInfoOfCopyRight({
            fb: `https://www.facebook.com/david.tu.587`, ig: `https://www.instagram.com/david.tu.guitar`, line: `davidtu0725`
        });
    }

    async function updateCPRTContent() {
        await api.submitInfoOfCopyRightContact({
            fb: `https://www.facebook.com/david.tu.587`, ig: `https://www.instagram.com/david.tu.guitar`, line: `davidtu0725`, phone: `+886982763479`, email: `freshingmoon0725@gmail.com`
        });
    }
    ...
    async function updateSingerOfSuggest() {
        ...
        const suggests = singers.map((singer) => {
            return {
                label: singer.name, uid: singer.id, id: singer.id, value: singer.id, popularLevel: singer.popularLevel
            };
        });
    ...
    async function updatePopularLevel() {
        ...
        for (const g of await api.fetchGuitarpus()) {
            ...
            if (gp !== tp) gp > tp ? (console.log(`中了！更新DB(${tp})->(${gp})`), await database.updateRecords("TONE", { popularLevel: gp }, new Builder().equal("idOfRemote", g.id).stmt())) : (console.log(`中了！更新遠端(${gp})->(${tp})`), await api.updateGuitarpuItem({ popularLevel: tp }, g.id));
        }
    }
    ...
    async function syncGuitarToneFromRemote() {
        ...
        console.log(guitars.map((each) => {
            return { name: each.name, id: each.id, idOfAuthor: each.idOfAuthor };
        }));
    ...
    async function syncToneIntoLocalSQLByEachID(...idOFTone) {
        for (const id of idOFTone) {
            ...
            content.popularLevel = guitar.popularLevel;
            content.singer = guitar.singer;
            content.url = Util.getRandomHash(20);
            console.log(content);
            await database.insertRecord("TONE", content);
