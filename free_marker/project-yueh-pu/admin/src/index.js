...
(async () => {
    ...
    function refactorTone(string) {
        ...
        let latest = Util.replaceAllWithSets(
            origin,
            { from: "▲", to: "🌕" },
            { from: "★", to: "🌓" },
            { from: "△", to: "🌑" },
    ...
    async function updatePopularLevel() {
        ...
        for (const g of await api.fetchGuitarpus()) {
            ...
            if (!tone) continue;
            const { popularLevel: gp } = g,
                { popularLevel: tp } = tone;
            if (gp !== tp)
                gp > tp
                    ? (console.log(`中了！更新DB(${tp})->(${gp})`), await database.updateRecords("TONE", { popularLevel: gp }, new Builder().equal("idOfRemote", g.id).stmt()))
                    : (console.log(`中了！更新遠端(${gp})->(${tp})`), await api.updateGuitarpuItem({ popularLevel: tp }, g.id));
        }
    }
