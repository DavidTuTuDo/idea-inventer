...
class Utiller {
    ...
    findFilePathBy = (path, predicate, ...excludes) => {
    ...
    async reinstallNodeModules(...exclude) {
        const ex = [...exclude, 'node_modules', 'utiller', 'configer'];
        /** utiller 不能刪掉,不然就爆了, configer是他的依賴也不能刪 */

        const paths = this.findFilePathBy('./', (dir) => _.isEqual(dir, 'package.json'), ...ex)
    ...
    async packageTemplatify(path, packageName) {
        const existFolders = this.getChildPathByPath(path).map((each) => each.split('/').pop());
        ...
        const classBase = String.format(this.readRawFile(`./template/sample.src.index.js`), packageName, '明悅', new Date());
        ...
        fs.writeFileSync(`${dirPath}/lib/index.js`, String.format(this.readRawFile(`./template/sample.warning.index.js`), packageName));

        /** 6.要產生webstorm run case? */
        const ideaWorkspacePath = `${this.findSpecificFolderByPath(dirPath, '.idea')}/workspace.xml`;

        fs.copyFileSync(ideaWorkspacePath, `${dirPath}/source_workspace.xml`); /** 怕改壞,先保留一份 */

        if (fs.existsSync(ideaWorkspacePath)) {
            const workspace = this.readRawFile(ideaWorkspacePath);
        ...
        path-to-js-file="$PROJECT_DIR$/${packageName}/src/index.js" 
        working-dir="$PROJECT_DIR$/${packageName}" >`,
                `    <envs>`,
    ...
    singleFileTemplatify(path = './') {
        ...
        for (const file of all) {
            ...
            if (_.isEmpty(content)) {
                ...
                fs.writeFileSync(file.absolute, String.format(this.readRawFile(`./template/sample.src.index.js`), className, '明悅', new Date()));
            }
        }
    }
    ...
    async generatePackage(path = './') {
        ...
        packagejsons = _.filter(packagejsons,
            (each) => _.isEqual(each.fileName, 'package') && this.has(this.readRawFile(each.absolute), 'gen'));
        packagejsons = packagejsons.map((each) => this.getFolderPathOfSpecificPath(each.absolute));
        for (const path of packagejsons) {
            await this.executeCommandLine(`cd ${path} && npm run gen`);
        }
    }
    ...
    async createPackageInTerminal(path = './') {
        ...
        if (!_.isEmpty(result.name)) {
            await this.packageTemplatify(path, result.name);
        } else {
...
const singleton = new Utiller();
export default singleton;
export {singleton as utiller, ERROR as exceptioner, pooller as pooller};
