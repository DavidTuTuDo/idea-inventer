...
| **Hosting (新)** | **初始化 Web 託管環境並自動產生設定檔，同時將版本儲存空間上限預設為 10 個。** | `gcloud firebase hosting sites update ...` |

* **Hosting 版本控制考量：** 為優化儲存空間利用率，腳本透過 `gcloud` 指令將預設的 40 個版本保留上限縮減為 **10 個**。這能自動清理過舊的部署紀錄，避免佔用不必要的雲端儲存配額。
...
* **延遲與權限傳播:** 腳本強制等待 **60 秒** (`sleep 60`)，以確保 SA 創建 and 權限綁定在全球範圍內充分傳播，避免 Admin SDK 因權限不足而初始化失敗。
...
| `.firebaserc` (新) | 儲存專案別名配置，確保 CLI 自動關聯正確的雲端專案。 | Firebase CLI |
| `public/` (新) | 預設的 Hosting 靜態檔案存放目錄。 | Firebase Hosting |
...
| **手動調整 Hosting 版本限制** | 前往 Firebase Console -> Hosting -> 版本紀錄，可手動將「保留的版本數量」設定在 1~1000 之間。 |
